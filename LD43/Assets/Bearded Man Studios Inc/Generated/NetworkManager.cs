...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] DropZoneNetworkObject = null;
		...
		public GameObject[] ServerSideNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is DropZoneNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (DropZoneNetworkObject.Length > 0 && DropZoneNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(DropZoneNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<DropZoneBehavior>();
						}
					}
			...
			else if (obj is ServerSideNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (ServerSideNetworkObject.Length > 0 && ServerSideNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(ServerSideNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<ServerSideBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateDropZone instead, its shorter and easier to type out ;)")]
		public DropZoneBehavior InstantiateDropZoneNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(DropZoneNetworkObject[index]);
			var netBehavior = go.GetComponent<DropZoneBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<DropZoneBehavior>().networkObject = (DropZoneNetworkObject)obj;
		...
		[Obsolete("Use InstantiateServerSide instead, its shorter and easier to type out ;)")]
		public ServerSideBehavior InstantiateServerSideNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(ServerSideNetworkObject[index]);
			var netBehavior = go.GetComponent<ServerSideBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<ServerSideBehavior>().networkObject = (ServerSideNetworkObject)obj;
		...
		public DropZoneBehavior InstantiateDropZone(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(DropZoneNetworkObject[index]);
			var netBehavior = go.GetComponent<DropZoneBehavior>();
		...
		public ServerSideBehavior InstantiateServerSide(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(ServerSideNetworkObject[index]);
			var netBehavior = go.GetComponent<ServerSideBehavior>();
