...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] SacrificeZoneNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is SacrificeZoneNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (SacrificeZoneNetworkObject.Length > 0 && SacrificeZoneNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(SacrificeZoneNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<SacrificeZoneBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateSacrificeZone instead, its shorter and easier to type out ;)")]
		public SacrificeZoneBehavior InstantiateSacrificeZoneNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(SacrificeZoneNetworkObject[index]);
			var netBehavior = go.GetComponent<SacrificeZoneBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<SacrificeZoneBehavior>().networkObject = (SacrificeZoneNetworkObject)obj;
		...
		public SacrificeZoneBehavior InstantiateSacrificeZone(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(SacrificeZoneNetworkObject[index]);
			var netBehavior = go.GetComponent<SacrificeZoneBehavior>();
